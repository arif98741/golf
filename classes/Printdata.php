...
class Printdata {
    ...
    public function SellReportForsales($starting,$ending) {
        ...
        $stmt = $this->dbObj->select("SELECT DISTINCT
                                    *
                                FROM
                                    tbl_sell_products tsp
                                JOIN tbl_product tp ON
                                    tsp.product_id = tp.product_id
                                JOIN tbl_sell ts ON
                                    tsp.sell_id = ts.sell_id
                                JOIN tbl_customer tc ON
                                    tsp.customer_id = tc.customer_id
                                JOIN tbl_group tg ON
                                    tp.product_group = tg.groupid
                                JOIN tbl_supplier tss ON
                                    tp.product_brand = tss.supplier_id      
                                WHERE
                                    ts.date BETWEEN '$starting' AND '$ending'
                                group by (tsp.serial_no)    
                                ORDER BY
                                    tsp.serial_no
                                DESC");
        $total = 0;
    ...
    public function SellReportByCustomer($starting,$ending,$customer_id) {
        ...
        $stmt = $this->dbObj->select("SELECT DISTINCT
                                ...
                                JOIN tbl_supplier tss ON
                                    tp.product_brand = tss.supplier_id      
                                WHERE
                                    tc.customer_id = '$customer_id' AND
                                    ts.date BETWEEN '$starting' AND '$ending'
                                group by (tsp.serial_no)    
                                ORDER BY
                                    tsp.serial_no
